...
def process_images(input_folder, output_folder, config):
    ...
    # Clasificar archivos manteniendo el orden numérico
    ...
    for filename in files['portadas']:
        ...
        try:
            with Image.open(input_path) as img:
                ...
                calidad = config['calidad_jpeg_portada'] if config['calidad_jpeg_portada'] is not None else 85
    ...
    # Procesar contraportadas (siempre al final)
    for filename in files['contraportadas']:
        ...
        try:
            with Image.open(input_path) as img:
                img = correct_exif_orientation(img)
                calidad = config['calidad_jpeg_contraportada'] if config['calidad_jpeg_contraportada'] is not None else 85
                img.save(output_path, 'JPEG', 
                         quality=calidad,
...
def generar_pdf(imagenes, output_pdf):
    ...
    try:
        # Separar las imágenes por tipo
        portadas = [img for img in imagenes if 'portada' in os.path.basename(img).lower() and 'contraportada' not in os.path.basename(img).lower()]
        internas = [img for img in imagenes if 'portada' not in os.path.basename(img).lower() 
                   and 'contraportada' not in os.path.basename(img).lower()]
        contraportadas = [img for img in imagenes if 'contraportada' in os.path.basename(img).lower()]
        
        # Orden final: portadas -> internas -> contraportadas
        imagenes_ordenadas = portadas + internas + contraportadas

        with open(output_pdf, "wb") as f:
            f.write(img2pdf.convert([img for img in imagenes_ordenadas if os.path.exists(img)]))
        return True
    except Exception as e:
...
class ConversorPDFApp:
    ...
    def crear_interfaz(self):
        ...
        self.spin_calidad_portada = tk.Spinbox(frame_portadas, from_=25, to=100, width=5)
        self.spin_calidad_portada.delete(0, tk.END)
        self.spin_calidad_portada.insert(0, "90")
        ...
        self.spin_calidad_contraportada = tk.Spinbox(frame_contraportadas, from_=25, to=100, width=5)
        self.spin_calidad_contraportada.delete(0, tk.END)
        self.spin_calidad_contraportada.insert(0, "90")
        self.spin_calidad_contraportada.config(state=tk.DISABLED)
        self.spin_calidad_contraportada.pack()
        
        # Botón de inicio
        self.boton_procesar = tk.Button(main_frame, text="Iniciar Procesamiento", 
    ...
    def seleccionar_internas(self, opcion):
        if opcion == "CCITT Group 4":
            self.root.after(0, lambda: messagebox.showwarning("Aviso", "Las imágenes internas se convertirán a blanco y negro"))
    ...
    def iniciar_procesamiento(self):
        ...
        try:
            ...
            self.actualizar_progreso(0, "Preparando procesamiento...")

            procesamiento_thread = threading.Thread(
                target=self.ejecutar_procesamiento_en_hilo,
                args=(carpeta_raiz, config),
                daemon=True
            )
    ...
    def ejecutar_procesamiento_en_hilo(self, carpeta_raiz, config):
        try:
            ...
            for folder_name in os.listdir(carpeta_raiz):
                ...
                if os.path.isdir(folder_path):
                    ...
                    if os.path.exists(input_folder):
                        ...
                        self.root.after(0, lambda: self.actualizar_progreso(
            ...
            for pdf in pdfs_generados:
                ...
                destino = os.path.join(carpeta_raiz, nombre_pdf)
                shutil.copy2(pdf, destino)

            self.root.after(0, lambda: self.actualizar_progreso(100, "Procesamiento completado"))
            self.root.after(0, lambda: messagebox.showinfo(
                "Resultado",
                f"Procesadas {procesadas} carpetas\n"
                f"Total imágenes convertidas: {imagenes_totales}\n"
                f"PDFs copiados a la carpeta raíz"
            ))
            self.root.after(0, lambda: self.boton_procesar.config(state=tk.NORMAL))
        
        except Exception as e:
            self.root.after(0, lambda: messagebox.showerror("Error", f"Ocurrió un error:\n{str(e)}"))
            self.root.after(0, lambda: self.actualizar_progreso(0, "Error en el procesamiento"))
            self.root.after(0, lambda: self.boton_procesar.config(state=tk.NORMAL))
